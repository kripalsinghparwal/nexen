...
def download_pdf(pdf_link, session):
    mapping_file = "pdf_url_mapping.json"

    # Generate hash first ✅
    file_hash = hashlib.sha256(pdf_link.encode()).hexdigest()
    output_file = f"High_court_data\\pdfs\\{file_hash}.pdf"
    ...
    # ✅ Skip if already downloaded (O(1) lookup)
    if file_hash in mapping:
        print("Skipped ⏭️ (Already downloaded)")
        return

    # Make request
    response = session.get(url=pdf_link)
...
def getjson(html_content, courtName, stateName, stateCode, petres_name, rgyear, session):
    try:
        ...
        for a_tag in soup.find_all('a', href=True):
            ...
            if len(link) > 20:
                ...
                pdf_link = 'https://hcservices.ecourts.gov.in/hcservices/'+link
                download_pdf(pdf_link, session)
                # case_data['link'] = download_pdf(pdf_link, session)
            else:
                case_data['link'] = None
        # Extract IA details
...
def get_inside_data(case_no, cino, court_code, courtName, stateName, stateCode, petres_name, rgyear, seen_data_set):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            getjson(html_content, courtName, stateName, stateCode, petres_name, rgyear, session)
        else:
...
# petres_name = "MAHAVIR COAL WASHERIES"
