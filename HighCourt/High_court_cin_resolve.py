...
def write_resolve_textfile(data,file):
    try:
        ...
        #                 today), 'a') as f:
        with open(
                f"D:\\Nexensus_Projects\\HighCourt\\scrape_file\\{rgyear}\\{petres_name}\\" + file.replace(".txt", "") + "_ResolvedFile_{}.txt".format(
                        today), 'a') as f:
...
def write_erorr_textfile(data,file):
    try:
        ...
        #                 today), 'a') as f:
        with open(
                    f"D:\\Nexensus_Projects\\HighCourt\\scrape_file\\{rgyear}\\{petres_name}\\" + file.replace(".txt", "") + "_NotResolvedError_{}.txt".format(
                        today), 'a') as f:
            f.writelines(post_data)
            f.writelines(',')
            f.writelines('\n')
